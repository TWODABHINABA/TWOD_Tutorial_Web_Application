...
const ChatBot = () => {
  const [messages, setMessages] = useState([
    { text: "Hi! What subject are you interested in learning?", sender: "bot" },
  ]);
  const [input, setInput] = useState("");
  const [isOpen, setIsOpen] = useState(false);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      let { aiReply, courses } = response.data;

      setMessages((prevMessages) => [
        ...prevMessages,
        { text: aiReply, sender: "bot", courses: courses || [] },
      ]);
    } catch (error) {
      console.error("❌ AI Error:", error.response?.data || error);
      setMessages((prevMessages) => [
        ...prevMessages,
        { text: "⚠️ AI is unavailable. Please try again later.", sender: "bot" },
      ]);
    }
  };
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-5 w-80 bg-white shadow-lg rounded-lg border border-gray-300">
          <div className="bg-orange-500 text-white px-4 py-3 flex justify-between items-center rounded-t-lg">
            <span className="font-semibold">AI Tutor Chat</span>
            <button className="text-lg font-bold" onClick={toggleChat}>×</button>
          </div>

          <div ref={chatRef} className="p-4 max-h-64 overflow-y-auto space-y-4">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`p-2 rounded-lg max-w-[90%] break-words whitespace-pre-line ${
                  msg.sender === "user" ? "bg-orange-100 text-right ml-auto" : "bg-gray-200 text-left mr-auto"
                }`}
              >
                {msg.sender === "bot" ? (
                  <>
                      ...
                      <div className="mt-2 space-y-2">
                        {msg.courses.map((course, idx) => (
                          <div key={idx} className="bg-orange-100 p-3 rounded-md">
                            ...
                            <span className="text-gray-800 font-semibold">{course.courseType}</span>
                            <a
                              href={`/courses/${course._id}`}
                              className="mt-2 bg-orange-500 text-white px-3 py-1 rounded-md hover:bg-orange-600 transition block text-center"
                            >
                              View Course
                            </a>
                          </div>
