  ...
  /* <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 shadow-md rounded-lg w-96 max-h-[80vh] flex flex-col">
            ...
            <div className="overflow-y-auto max-h-[60vh] mt-2 px-1">
              ...
              newAvailableDates.length > 0 ? (
                [...filteredAvailability, ...newAvailableDates].map(
                  (dateObj, index) => (
                    <div key={index} className="mb-4 border p-2 rounded">
                      <div className="flex justify-between">
                        <h3 className="font-semibold">{dateObj.date}</h3>
                        <button
                          className="text-red-500 p-1"
                          onClick={() =>
                            handleRemoveDate(dateObj.date, selectedSubject)
                          }
                        >
                          ❌
                        </button>
                      </div>
                      {dateObj.subjects?.length > 0 &&
                        dateObj.subjects.map((subject, idx) =>
                          subject.subjectName === selectedSubject ? (
                            subject.timeSlots.length > 0 ? (
                              subject.timeSlots.map((slot, slotIdx) => (
                                <span
                                  key={slotIdx}
                                  className="bg-gray-200 p-1 m-1 rounded inline-block"
                                >
